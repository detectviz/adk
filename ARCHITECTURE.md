...
![ADK 高階架構圖](./docs/references/adk-architecture/adk-high-level-architecture.png)
...
## 11. SRE Assistant 專案特定實現 (SRE Assistant Project-Specific Implementation)

本章節旨在將前述的通用 ADK 架構原則與 **SRE Assistant** 專案的具體工程實踐聯繫起來，作為連接「理論」與「現實」的橋樑。

### 11.1 核心工作流程：`SREWorkflow`

本專案的核心業務邏輯由 `SREWorkflow` 協調器 (`src/sre_assistant/workflow.py`) 驅動。

-   **架構模式**: `SREWorkflow` 採用了**協調器-工作者模式 (Orchestrator-Worker)**。它本身是一個高階代理，其內部組裝了一個 `SequentialAgent` 來依序執行 SRE 事件回應的完整生命週期。
-   **執行序列**: 當前的工作序列嚴格遵循以下階段：
    1.  **診斷 (Diagnostics)**: 由 `CitingParallelDiagnosticsAgent` 負責，此代理會並行調用工具（如 `PrometheusQueryTool`, `LokiLogQueryTool`）來收集事件相關數據。
    2.  **修復 (Remediation)**: 由 `SREIntelligentDispatcher` 負責。這是一個智能分診器，它會根據診斷階段的結果，動態地選擇最合適的修復策略或工具。
    3.  **覆盤 (Postmortem)**: 事件處理完畢後，由 `PostmortemAgent` 負責生成覆盤報告初稿。
    4.  **優化 (Optimization)**: 最後，由 `IterativeOptimization` 代理對生成的內容進行迭代式的精煉。
-   **關注點分離**: `SREWorkflow` 在執行核心序列之前，會先獨立執行認證 (`authenticate`) 和授權 (`check_authorization`) 檢查，將安全控制與業務邏輯清晰地解耦。

### 11.2 工具管理：`VersionedToolRegistry`

為了滿足生產級環境對穩定性的嚴苛要求，本專案實現了一個自訂的、支援版本管理的工具註冊表 `VersionedToolRegistry` (`src/sre_assistant/tool_registry.py`)。

-   **設計動機**: 解決標準 ADK 工具註冊表在處理嚴格的版本依賴和環境相容性方面的不足。
-   **核心功能**:
    -   **版本化註冊**: 允許同一工具（如 `PrometheusQueryTool`）的多個版本並存於註冊表中。
    -   **環境相容性檢查**: 能夠根據當前部署環境的外部服務版本（如 Kubernetes API v1.25, Prometheus v2.40），動態選擇最合適的工具版本。
-   **架構決策**: 這是一個**策略性的技術決策**。我們選擇了自訂實現以換取更高的可靠性，但這也意味著它與 ADK 的原生工具註冊方式有所不同，是本專案的一個關鍵架構特點。

### 11.3 會話與記憶體管理實踐

-   **會話管理 (Session Management)**:
    -   **目標實現**: 根據 `TASKS.md` (`TASK-P1-CORE-02`) 的規劃，本專案的會話狀態將使用 ADK 的 `DatabaseSessionService` 進行持久化，後端儲存為 **PostgreSQL**。
    -   **現狀**: 在 MVP 階段，可能暫時使用 `InMemorySessionService` 進行開發和測試。
-   **長期記憶體 (Long-term Memory)**:
    -   **實現**: 本專案透過自訂的 `MemoryProvider` 連接到 **Weaviate** 向量數據庫，以實現 RAG 功能。
    -   **應用**: 主要用於儲存和檢索歷史事件的解決方案、SOPs 和覆盤報告，為代理的決策提供上下文支援。

### 11.4 回調 (Callbacks) 機制應用

本專案利用 ADK 的回調機制來實現橫切關注點 (Cross-Cutting Concerns) 的處理。

-   **實現模式**: 在 `SREWorkflow` 中，通過實現 `before_agent_callback` 和 `after_agent_callback` 函數，來注入工作流程開始前和結束後的通用邏輯。
-   **具體應用**:
    -   **前置檢查 (`_workflow_pre_check`)**: 在工作流程開始前，用於執行速率限制、權限二次檢查等。
    -   **後續處理 (`_workflow_post_process`)**: 在工作流程結束後，用於觸發審計日誌記錄、更新監控指標等。

### 11.5 理想與現實：目標設計的標示

本架構文件描述了許多本專案的**目標設計 (Target Design)**。為了清晰起見，特此說明，部分功能在當前 MVP 階段尚未完全實現，其開發狀態由 [TASKS.md](./TASKS.md) 追蹤。

-   **範例 - 標準化工具輸出**: `SPEC.md` 中定義了所有工具應返回標準化的 `ToolResult` Pydantic 模型。根據 `TASKS.md` (`TASK-P1-DEBT-02`)，這是一項計畫在後續版本中完成的技術債。