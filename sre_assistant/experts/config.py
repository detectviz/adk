...
def build_agent(tool_objs: Dict[str, Any]) -> "LlmAgent":
    """建立 ConfigExpert 實例。
    - 模型選擇：experts.config.model > agent.model > ADK_MODEL。
    - 工具清單：experts.config.tools_allowlist 或預設 ['ingest_text']。
    """
    if LlmAgent is None:
        raise RuntimeError("缺少 google-adk 套件，無法建立 ConfigExpert")
    cfg = load_adk_config()
    agent_cfg = (cfg.get("agent") or {})
    exp_cfg = ((cfg.get("experts") or {}).get("config") or {})
    model = exp_cfg.get('model') or agent_cfg.get('model') or os.getenv('ADK_MODEL', 'gemini-2.0-flash')
    tools = _select_tools(tool_objs, ["ingest_text"], "config")
    return LlmAgent(
        name="ConfigExpert",
        model=model,
        prompt = exp_cfg.get("prompt")
    instruction=prompt or '你是配置專家，生成與調整儀表板、告警與監控策略。' ,
        tools=tools,
    )
