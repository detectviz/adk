...
| [潛在客戶開發代理 (Lead Generation Agent)](leadGenerationAgentADK-main) | 透過發現成功公司的投資模式來尋找新的潛在客戶。 | `Multi-agent`, `Workflow`, `ADK`, `Pattern Discovery`, `Lead Generation` | 對話式 (Conversational) | 進階 (Advanced) | 多代理 (Multi Agent) | 銷售 / 行銷 (Sales / Marketing) |
...
| [NavigoAI 語音代理 (NavigoAI Voice Agent)](NavigoAI_Voice_Agent_ADk-main) | 使用 Gemini 的原生音訊功能和網頁介面的即時多模態語音助理。 | `Multimodal`, `Voice`, `Real-time`, `ADK`, `WebSocket`, `Gemini` | 語音 / 對話式 (Voice / Conversational) | 進階 (Advanced) | 單一代理 (Single Agent) | 旅遊 / 助理 (Travel / Assistant) |
